...
pb 的每个字段都有一个 tag, 其值为 `(filed_num << 3) + wire_type`, filed\_num 就是 proto 里等号后面的数字
...
| 2         | LENGTH\_DELIMITED (长度限定)        | `string`, `bytes`, 嵌套消息 (message), 打包重复字段                      |
| 3         | START\_GROUP (分组开始，Protobuf 2) | 不适用, Protbuf3 已弃用                                                  |
| 4         | END\_GROUP (分组结束，Protobuf 2)   | 不适用, Protbuf3 已弃用                                                  |
